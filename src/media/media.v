...
fn join(cmd Command) ! {
  ...
  if cmd.args.all(it.ends_with('.mp4')) ||
     cmd.args.all(it.ends_with('.mp3')) {
    ...
    if os.exists(outfile) { os.rm(outfile)! }
...
fn split_on_silence(cmd Command) ! {
  ...
  defer {
    os.rm(tmp_path) or {
      log.error('unable to remove tmp file $tmp_path')
    }
  }
...
fn resize(cmd Command) ! {
  ...
  for file in input_files {
    if (file_ext(file) or {''}) !in vid_file_types { continue }
